            ...
            if((context as TaskListActivity).mAssignedMemberDetailList.size>0){
                ...
                if(selectedMembersList.size>0){
                    if(selectedMembersList.size == 1 && selectedMembersList[0].id ==model.createdBy){
                        holder.itemView.requireViewById<RecyclerView>(R.id.rv_card_selected_members_list).visibility= View.GONE
                    }else{
                        holder.itemView.requireViewById<RecyclerView>(R.id.rv_card_selected_members_list).visibility= View.VISIBLE

                        holder.itemView.requireViewById<RecyclerView>(R.id.rv_card_selected_members_list).layoutManager=
                            GridLayoutManager(context, 4)
                        val adapter = CardMemberListItemsAdapter(context, selectedMembersList, false)
                        holder.itemView.requireViewById<RecyclerView>(R.id.rv_card_selected_members_list).adapter=adapter
